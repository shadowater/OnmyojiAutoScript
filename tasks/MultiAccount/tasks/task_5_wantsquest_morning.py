...
class Task5WantsquestMorning(BaseMultiAccountTask):
    """任务5: 早晨刷勾协"""
    
    def __init__(self):
        account_file = "D:\\software\\yys\\resource\\task_account\\account_info_wantsquest.json"
        super().__init__(account_file, "任务5-早晨刷勾协")
    ...
    def execute_task_for_account(self, account_info):
        ...
        try:
            ...
            logger.info(f"账号 {账号}-{角色} 早晨勾协任务完成")
        except Exception as e:
            logger.error(f"账号 {账号}-{角色} 早晨勾协任务失败: {e}")
